import { Mail, Phone, MapPin, Globe, Linkedin, Trophy } from "lucide-react";
...
const SportsTemplate = ({ 
  ...
  return (
    <div className={`max-w-4xl mx-auto bg-white p-8 shadow-lg min-h-[297mm] resume-wrapper ${isPDFMode ? 'tight-spacing' : ''}`}>
      {/* Sports Header */}
      <header className="bg-gradient-to-r from-red-600 to-orange-600 text-white p-8 rounded-lg mb-8 no-break">
          ...
          <div>
            <h1 className="text-4xl font-bold mb-2">
              <EditableText
                ...
                placeholder="Sports Professional"
                isEditing={isEditing}
            ...
            {personalInfo?.jobTitle && (
              <p className="text-xl opacity-90 font-medium">
                <EditableText
                  ...
                  isEditing={isEditing}
                  className="inline-block text-white"
                  placeholder="Athletic Professional"
                />
              </p>
            )}
          </div>
      ...
      {personalInfo?.summary && (
        <section className="mb-8 no-break">
          <h2 className="text-xl font-bold text-red-600 mb-4 flex items-center">
            ...
            Athletic Profile & Philosophy
          </h2>
          <div className="text-gray-700 leading-relaxed bg-red-50 p-6 rounded-lg border-l-4 border-red-500">
            <EditableText
              ...
              placeholder="Your athletic background and sports philosophy"
            />
          </div>
        </section>
      )}
      ...
      {experience?.length > 0 && (
        <section className="mb-8 section-break">
          <h2 className="text-xl font-bold text-red-600 mb-6 border-b-2 border-red-300 pb-2">
            Athletic & Coaching Experience
          </h2>
          <div className="space-y-6">
            {experience.map((exp: any, index: number) => (
              <div key={index} className="bg-red-50 p-6 rounded-lg border border-red-200 allow-break">
                ...
                <div className="text-gray-700 leading-relaxed whitespace-pre-line">
                  <EditableText
                    ...
                    isEditing={isEditing}
                    className="inline-block w-full"
                    placeholder="Athletic achievements and coaching accomplishments"
                  />
                </div>
              </div>
            ))}
      ...
      {skills?.length > 0 && (
        <section className="mb-8 no-break">
          <h2 className="text-xl font-bold text-red-600 mb-6 border-b-2 border-red-300 pb-2">
            Athletic Skills & Competencies
          </h2>
          <div className="grid grid-cols-3 gap-4">
            {skills.map((skill: string, index: number) => (
              <div key={index} className="bg-red-100 text-red-800 px-4 py-3 rounded-lg text-center font-medium border border-red-300">
                <EditableText
                  value={skill || ''}
                  onSave={(value) => {
              ...
              <div key={index} className="bg-red-50 p-6 rounded-lg border border-red-200 allow-break">
                <h3 className="text-lg font-bold text-gray-900 mb-2">
                  <EditableText
                    ...
                    isEditing={isEditing}
                    className="inline-block"
                    placeholder="Athletic Program/Achievement"
                  />
                </h3>
                <div className="text-gray-700 leading-relaxed whitespace-pre-line">
                  <EditableText
                    ...
                    isEditing={isEditing}
                    className="inline-block w-full"
                    placeholder="Program development and athletic outcomes"
                  />
                </div>
                {project.technologies && (
                  <p className="text-red-600 mt-2 font-medium">
                    Training Methods: {project.technologies}
                  </p>
                )}
              </div>
            ))}
        ...
        <section className="mb-8 no-break">
          ...
          <div className="space-y-4">
            {education.map((edu: any, index: number) => (
              <div key={index} className="flex justify-between items-center bg-red-50 p-4 rounded-lg no-break">
                <div>
                  <h3 className="text-lg font-bold text-gray-900">
                    <EditableText
                      value={edu.degree || ''}
                      ...
                      isEditing={isEditing}
                      className="inline-block"
                      placeholder="Sports/Kinesiology Degree"
                    />
                  </h3>
                  <p className="text-red-600 font-semibold">
                    <EditableText
                      ...
                      isEditing={isEditing}
                      className="inline-block"
                      placeholder="Sports Institution"
                    />
                  </p>
                </div>
